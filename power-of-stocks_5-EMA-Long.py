# Algo Trading with Himmat
    # Strategy  - 5 EMA (Power of Stocks) - 15 Min Candle
    # Entry     - High of previous candle should not touch 5 ema and current candle price crosses above high of previous candle 
    # SL        - Low of the candle
...
def process_data():    
    ...
    if candle_high < candle_ema and ltp > candle_high and not is_trade:
        ...
        target              = candle_high + ((candle_high - candle_low) * 2)
        print(str(time.localtime().tm_hour)+':'+str(time.localtime().tm_min)+':'+str(time.localtime().tm_sec)+\
...
while True:
    ...
    historicData    = kite.historical_data(instrument_token=BNInstrumentToken, from_date=from_date, to_date=to_date, interval='15minute')
    ...
    t_end           = time.time() + (15 * 60) # 15 mins until next candle formed
